...
CREATE TABLE `package_item_rel` (
  ...
  `item_code` char(10) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '物件编号',
  `create_at` datetime NOT NULL COMMENT '关联创建时间',
  `operator` int NOT NULL COMMENT '操作者（用户id）',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='包裹物件关联';
